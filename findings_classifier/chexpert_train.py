...
def save_preds(dataloader, split):
    # load checkpoint
    ckpt_path = f"findings_classifier/checkpoints/train_chexbert/ChexpertClassifier-epoch=01-val_f1=0.32.ckpt"
...
if __name__ == '__main__':
    ...
    if TRAIN:
        ...
        if not isinstance(class_weights, torch.Tensor):
            ...
            class_weights = class_weights.clone().detach().to(dtype=torch.float32)
        # Define the model
        lit_model = LitIGClassifier(num_classes, class_names, class_weights=class_weights, learning_rate=args.lr)
